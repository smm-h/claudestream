...
The spend guard that arms the suite -- the integration opt-in and the poisoned
``claude`` shim -- is deliberately NOT here. It lives in the repository-root
``conftest.py`` so that it covers every pytest invocation rooted at this
repository, not only the ones that reach into ``tests/``. See that file and
